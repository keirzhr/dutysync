...
function validateDropdownValues(dayType) {
    const validDayTypes = ['Regular', 'Regular Holiday (200%)', 'Special Non-Working Holiday (130%)'];
...
function createDayElement(day, month, year, isOtherMonth) {
    ...
    if (allDuties.some(d => d.date === dateStr)) el.classList.add('has-duty');
    el.textContent = day;

    if (!isOtherMonth) el.addEventListener('click', () => openPopup(dateStr));
    return el;
}
...
function calculateCutoffHours(duties) {
    ...
    duties.forEach(d => {
        ...
        if (day <= 15) hours_1_15 += totalPayableTime;
        else hours_16_end += totalPayableTime;
    });
